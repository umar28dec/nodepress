...
{
	var ele = $(document).find('.bulk_post_id');
...
$(document).on('click', '.delete_selected_posts', function(event) 
{
	...
	if(_.size(get_post_ids) < 1)
	{
		...
		$(document).find('.posts_alert').html(d)
	}
	...
	{		
 		...
 		var ObjArrOptions = 
		{
		  ...
		  text: "Are you sure you want to delete "+_.size(get_post_ids)+" post(s) ?",
...
var delete_test = function (delete_post_ids_arr)
{
	...
	delete_multiple_posts(get_post_ids_to_be_deleted)

}

var delete_multiple_posts = function(delete_post_ids)
{

	var CallType   = 'DELETE' //--->Other options: GET/POST/DELETE/PUT
	var AjaxURL    = 'posts/multiple';
	var DataString = 
	{		 
		datasend:'files',
